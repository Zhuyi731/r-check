define(function (require, exports, module) {
	...
	function Lan6CfgModule() {
		...
		this.initEvent = function () {
			...
			$("input").on("blur",mainLogic.initModuleHeight);
			$("#submit").on("click",mainLogic.initModuleHeight);//清除提示语对高度的影响
			$("#ipv6PPPoeTemporary,#ipv6PPPoeDelegation").on("click",this.disablePrefix);
			$("#ipv6WanType").on("click",this.disablePrefix);
		};
		this.initValue = function (obj) {
		...
		this.disablePrefix = function(){
			if($("#ipv6WanType").val() == "0"){
				if($("#ipv6PPPoeTemporary")[0].checked == true &&　$("#ipv6PPPoeDelegation")[0].checked == true ){ //两者都勾选的情况
					$("#ipv6LanDhcpPrefixType").val("0").attr("disabled","disabled");
					$("#ipv6LanDnsType").removeAttr("disabled");
				}else if($("#ipv6PPPoeDelegation")[0].checked == false ){//Temporary勾选 Delegation不勾选
					$("#ipv6LanDhcpPrefixType").val("1").attr("disabled","disabled");
					$("#ipv6LanDnsType").val("1").attr("disabled","disabled");
