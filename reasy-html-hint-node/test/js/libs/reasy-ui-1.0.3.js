...
(function (window, document) {
	...
	valid = {
		...
		ipv6: {
			...
			specific: function (str) {
				...
				if (headStr.slice(0, 2) == "ff") {
					return (_("Insira um endereço válido."));
				}
		...
		ascii2: function (str, min, max) {
			if (!(/^[ -~]+$/g).test(str)) {
				return _("Must be ASCII2.");
			}
			if (min || max) {
				return valid.len(str, min, max);
			}
		},
		ascii3: function (str, min, max) {//for wifi ssid and pwd
			if (!(/^[ -~]+$/g).test(str)) {
				return _("Invalid wifi password");
			}
			if (min || max) {
				return valid.len(str, min, max);
			}
		...
		ssidPasword2: function (str, minLen, maxLen) {
			var ret;
			ret = $.validate.valid.ascii2(str);
			if (!ret && minLen && maxLen) {
				ret = valid.len(str, minLen, maxLen);
				if (ret) {
					return ret;
		...
		pppoeUser: {
			all: function (str) {
				...
				if (!(/^[ -~]+$/g).test(str)) {
					return _("Invalid pppoe username.");
				}
			},
			specific: function (str) {
				if ((/[\\'"]/g).test(str)) {
		...
		pppoePassword: {
			all: function (str) {
				...
				if (!(/^[ -~]+$/g).test(str)) {
					return _("Invalid pppoe password.");
				}
			},
			specific: function (str) {
				if ((/[\\'"]/g).test(str)) {
