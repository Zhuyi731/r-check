define(function (require, exports, module) {
	...
	function AttachedModule() {
		...
		this.initEvent = function () {
			...
			$("#qosList").delegate(".dropdown input[type='text']", "blur.refresh", function () {
				...
				if (isNaN(parseInt(this.value, 10))) {
					...
					if (parseInt(this.value) > 307200) { //大于307200Kbps则表示无限制 38400*8
						this.value = _("No Limit");
						...
						this.value = parseInt(this.value, 10) + _("Kbps");
		...
		function createOnlineList(obj) {
			...
			var selectDownObj = {
					...
					"options": [{
						...
						"1024": _("1024 Kbps(Web Browsing)")
					}, {
						"2048": _("2048 Kbps(SD Videos)")
					}, {
						"4096": _("4096 Kbps(HD Videos)")
					}, {
						".divider": ".divider"
					}, {
						".hand-set": _("Manual(unit: Kbps)")
					}]
				},
				selectUpObj = {
					"initVal": "21",
				...
				for (prop in obj.onlineList[i]) {
					...
					if (prop == "qosListHostname") { //主机名
						...
						if (prop == "qosListUpLimit") {
							upLimit = obj.onlineList[i][prop] + _("Kbps");
							if (+obj.onlineList[i][prop] >= 307201) { //原值为38401 由KB/s修改为Kbps后38400*8+1结果为307208
								upLimit = ("No Limit");
							}
						} else {
							downLimit = obj.onlineList[i][prop] + _("Kbps");
							if (+obj.onlineList[i][prop] >= 307201) {
								downLimit = ("No Limit");
							}
						}
		...
		function shapingSpeed(value) {
			...
			if (val > 1024) {
				return (val / 1024).toFixed(2) + _("Mbps");
			} else {
				return val.toFixed(0) + _("Kbps");
			}
		}
