        ...
        $("#ok").on("click", function () {
            ...
            $.ajax({
                ...
                success: function (obj) {
                    ...
                    //client+ap或ap模式下，重启后跳转至meuintelbras.com.br
                    if (pageModule.mode == "client+ap" || pageModule.mode == "ap") {
                        progressLogic.init("", "reboot", 200, "meuintelbras.local");
                    } else {
                        //重启后刷新当前页面
    ...
    pageModule.beforeSubmit = function () {
        ...
        if (mode == exMode && (exMode == "disabled" || exMode == "ap")) {
            ...
            case "ap":
                ...
                $("#dialog-head-title").html(_("Reboot0"));
                msg = _("If you enable AP mode, the router will reboot. When the router finishes reboot process, please use meuintelbras.local to log in to the web UI.");
                break;
            case "client+ap":
                $("#m-dialog").css("height", "320px");
                $("#dialog-head-title").html(_("Reboot2"));
                msg = _("When you enable the Universal Repeater mode, the router reboots. After the router finishes the reboot process, use meuintelbras.local to log in to the web UI.");
                break;
            case "wisp":
                $("#m-dialog").css("height", "320px");
                $("#dialog-head-title").html(_("Reboot1"));
